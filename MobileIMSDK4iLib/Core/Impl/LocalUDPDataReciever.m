...
- (void) handleProtocal:(NSData *)originalProtocalJSONData
{
    ...
    if(pFromServer.QoS)
    {
        ...
        {
            //获取会话id
            NSString *content = pFromServer.dataContent;
            ...
            if (content.length) {
                ...
                NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData
                                                                    options:kNilOptions
                                                                      error:&error];
                if (!error) {
                    pFromServer.chatId = dic[@"conversationId"];
                }
            }
    ...
    switch(pFromServer.type)
    {
        case FROM_CLIENT_TYPE_OF_COMMON_DATA:
        {
            ...
            {
                [[ClientCoreSDK sharedInstance].chatTransDataEvent onTransBuffer:pFromServer.fp withUserId:pFromServer.from andContent:pFromServer.dataContent andTypeu:pFromServer.typeu protocal:pFromServer];
            }
            break;
        }
        ...
        case FROM_CLIENT_TYPE_OF_RECIVED:
        {
            ...
            if([ClientCoreSDK sharedInstance].messageQoSEvent != nil)
                [[ClientCoreSDK sharedInstance].messageQoSEvent messagesBeReceived:theFingerPrint protocal:pFromServer];
        ...
        case FROM_SERVER_TYPE_OF_RESPONSE_LOGIN:
        {
            ...
            if(loginInfoRes.code == 0)
            {
                ...
                ObserverCompletion observerBlock = ^(id observerble ,id data) {
                    ...
                    [[ClientCoreSDK sharedInstance].chatBaseEvent onLinkCloseMessage:-1 protocal:pFromServer];
            ...
            {
                [[ClientCoreSDK sharedInstance].chatBaseEvent onLoginMessage:loginInfoRes.code protocal:pFromServer];
            }
        ...
        case FROM_SERVER_TYPE_OF_RESPONSE_FOR_ERROR:
        {
            ...
            if([ClientCoreSDK sharedInstance].chatTransDataEvent != nil)
            {
                [[ClientCoreSDK sharedInstance].chatTransDataEvent onErrorResponse:errorRes.errorCode withErrorMsg:errorRes.errorMsg protocal:pFromServer];
            }
            break;
        }
        ...
        default:
        {
            if([ClientCoreSDK sharedInstance].chatTransDataEvent != nil)
            {
                [[ClientCoreSDK sharedInstance].chatTransDataEvent onTransOtherBuffer:pFromServer.fp content:pFromServer.dataContent andTypeu:pFromServer.typeu protocal:pFromServer];
            } else {
...
- (void) sendRecievedBack:(Protocal *)pFromServer
{
    ...
    {
        Protocal *p = [ProtocalFactory createRecivedBack:pFromServer.to toUserId:pFromServer.from withFingerPrint:pFromServer.fp andBridge:pFromServer.bridge chatId:pFromServer.chatId];
